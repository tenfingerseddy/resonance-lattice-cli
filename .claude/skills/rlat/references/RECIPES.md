...
Auto-detect sources, build a cartridge, and generate an assistant primer in one step:
...
For larger corpora, compress the cartridge and registry:
...
- External store (no text in cartridge): `--store-mode external`
...
## Keeping Cartridges Updated

### Incremental sync after file changes

After editing source files, sync the cartridge:
...
### Merge team cartridges

```bash
# Combine two team cartridges
...
Build a lightweight cartridge that reads evidence from local files at query time:
...
Useful for: reducing cartridge size, avoiding embedded PII, keeping source text always up-to-date.
