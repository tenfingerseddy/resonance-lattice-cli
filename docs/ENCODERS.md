...
Resonance Lattice uses a transformer encoder to convert text into semantic vectors during build and query. The encoder path affects retrieval quality, build speed, language coverage, warm-path behavior, and cartridge compatibility.

## What Do Encoders Do?

The encoder is the front door into the cartridge. It defines how text is projected into the phase-space representation used by the field, the registry, and the query path.
...
| Preset | Backbone | Params | Context | Pooling | Best For |
|--------|----------|--------|---------|---------|----------|
| `codesage-large` | codesage/codesage-large-v2 | 1.3B | 2K | mean | Code-heavy repos, NL→code retrieval |
| `gte-qwen2-1.5b` | Alibaba-NLP/gte-Qwen2-1.5B-instruct | 1.5B | 8K | mean | Best general quality ≤1.5B, code in training mix |
| `jina-v3` | jinaai/jina-embeddings-v3 | 570M | 8K | mean | Task-specific LoRA adapters, good code+prose balance |
...
Use `codesage-large` or `gte-qwen2-1.5b`. CodeSage was specifically trained for natural language → code retrieval. GTE-Qwen2 is a strong all-rounder with code in its training data.

**Mixed code and documentation**:
Use `gte-qwen2-1.5b` or `qwen3-0.6b`. These handle both code and prose well in a single embedding space.
...
The cartridge format and query interface stay the same regardless of which encoder you use.

## How Encoders Bind to Cartridges

**One encoder per cartridge.** The encoder is locked at build time and its fingerprint is stored in the cartridge manifest.

- `rlat add` and `rlat sync` verify that the current encoder matches the one used to build the cartridge
- If there is a mismatch, the CLI will refuse to proceed and tell you which encoder the cartridge expects
- Changing the encoder means rebuilding the cartridge
...
**CodeSage** has a 2048-token context limit — shorter than other options. This is rarely a problem for code because the AST-aware chunker (Python) and text chunker (other languages) already split files into smaller chunks. But very long docstrings or configuration files may be truncated.
...
For mixed code and documentation corpora, use **jina-v3** (requires `pip install einops`). For a zero-dependency upgrade from E5, use **gte-large**.
