...
Compose multiple cartridges at query time without modifying the underlying files. All composition modes produce queryable objects with full provenance tracking.
...
| `ComposedCartridge.contradict(set_a, set_b)` | Surface disagreements between cartridges |
...
# results[0].cartridge == "docs" or "code"
...
Boost or suppress topics on any composed cartridge without rebuilding.
...
Control how each cartridge's results are framed for an LLM.
...
# Each result carries: composed.get_injection_mode(result.cartridge)
```
...
## Cross-Cartridge Cascading

**Module:** `resonance_lattice.network`

Multi-hop search following an explicit route across cartridge boundaries.
...
Query-aware dynamic weighting across cartridges.
...
| `band` | Per-band routing: each band selects the strongest cartridge | Heterogeneous corpora |
...
Makes cartridges discoverable to AI assistants without manual configuration.

### Manifest

The manifest at `.rlat/manifest.json` indexes all cartridges in the project:
...
update_mcp_json()       # wire MCP server to primary cartridge
inject_claude_md()      # inject cartridge section into CLAUDE.md
```
...
# [("docs", 4.2), ("code", 2.1)]  — docs is the better cartridge for this query
```
...
| Tool | Description |
|------|-------------|
| `rlat_discover` | List all available cartridges with domain, freshness, and primer paths |
| `rlat_freshness` | Check if a specific cartridge needs rebuild |
...
|------|----------|-------------|
| `--with` | `PATH` | Merge with additional cartridge (repeatable) |
| `--through` | `PATH` | Project through lens cartridge |
...
| `--cascade-through` | `PATH...` | Multi-hop cascade through listed cartridges |
