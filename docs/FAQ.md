...
- your `.rlat` cartridge makes the broader corpus queryable to tools and assistants

They work best together, not as substitutes for each other.

### How does it work?

`rlat build` chunks your files, encodes each chunk with a local encoder, and packages everything into a single `.rlat` cartridge. The cartridge has three layers:
...
| **external** | Lightweight `.rlat` with field and registry only; evidence is read from your local source files at query time | Smaller cartridge, always up-to-date text, but tied to the source directory |
...
1. build one cartridge from your docs and code
...
5. refresh the cartridge with `rlat build` or `rlat sync` as the project changes
...
Resonance Lattice is fundamentally a mathematical system, not a generative one. For users, that means the cartridge behaves predictably when you add, remove, merge, or compare sources. The field has algebraic structure, and every operation — adding a source, removing a source, merging two cartridges, diffing versions — is a defined mathematical operation with known properties.
...
- **Composability** — merge is order-independent. The sequence you combine cartridges does not change the result.
...
- the cartridge workflow should survive encoder changes
...
Trained-head checkpoints exist experimentally, but they are **not** the recommended default. The CLI reference notes that trained heads are not yet recommended because build and query behavior diverged in testing, so the recommended setup remains the standard production encoder without a checkpoint.
...
- **Version comparison** — compare the knowledge in two cartridges to see what changed
...
- **Cartridge algebra** — merge, diff, and forget knowledge artifacts with mathematical guarantees
- **Semantic profiling** — inspect a cartridge's health, coverage, and structure
...
- **Single-file portable** — one `.rlat` cartridge you can move like any other file
...
A useful way to think about it: skills today load the same static document regardless of the query. Most of that context is irrelevant to any given request. With RL **cartridge-backing**, a skill declares which cartridges it draws from, and context adapts to the specific question:
...
**Benchmark methodology**: 50 evaluation questions against a 2,266-file Fabric documentation corpus. For each question, we extracted keywords and grep-matched the top 5 files by keyword count, then read those files in full and counted tokens. `rlat search` ran `rlat search --format prompt --top-k 10` on the same questions against a cartridge built from the same corpus. Token counts via tiktoken (`cl100k_base`). Median compression was `19.9x`; all 50 questions produced valid comparisons. Full results live in the repo's `benchmarks/results/token_efficiency/` directory.
...
The cartridge is a portable file, not a service dependency. And you get operations standard RAG setups usually do not offer cleanly:
...
That is important because it shows where the quality actually comes from. The main gain comes from the cartridge + pipeline + assistant workflow, not from dense retrieval alone.
...
It is not a full rebuild. The cartridge tracks every file by content hash — when you run `rlat build` or `rlat sync`, only changed, added, or removed files are processed. Unchanged files are skipped entirely.
...
- **cold** — first query, loads the cartridge and encoder from disk
...
- the cartridge is already loaded
- the encoder is already loaded
- the background worker is already running

That matters in assistant workflows because repeated queries inside one coding session should feel interactive. This is one reason the MCP server matters: it keeps the cartridge hot instead of paying repeated startup cost.
...
| `external` | Field + registry only | Smaller cartridge; pass `--source-root` at query time |
...
1. `--checkpoint` flag (trained projection heads)
2. `--encoder` flag (preset name or HuggingFace model ID)
3. stored encoder in the cartridge
4. default encoder (`e5-large-v2`)
...
Most users never need `--encoder`. The cartridge stores its encoder at build time, and queries restore it automatically.
...
- keep the cartridge bound to that encoder
...
The important point is that these are backbone and pipeline trade-offs, not workflow trade-offs. The `.rlat` cartridge remains the stable user-facing abstraction.
...
The CLI supports `--onnx`, and it can also auto-detect an ONNX directory alongside the cartridge in common cases.
...
This is a good option when cartridge size matters more than the last bit of retrieval fidelity.

### Can I compress the cartridge file itself?

Yes. The build command supports cartridge compression:
...
The primary value remains the cartridge and query workflow:
...
- **`profile`** — inspect what the cartridge appears to know
- **`compare`** — compare semantic shape across two cartridges
...
### What is a cartridge-backed skill?

A regular skill is a static document — the same context loads every time. A cartridge-backed skill has a live knowledge connection. It declares which `.rlat` cartridges it draws from, and RL adapts the injected context to each request.
...
They are defined in the `cartridge-queries` frontmatter field. When the skill triggers, these queries resonate against the declared cartridges and pull ranked passages — the same retrieval pipeline as `rlat search`, but automated and budget-capped (40% of the token budget by default).
...
The practical effect: skill authors guarantee baseline domain knowledge without bloating SKILL.md with thousands of tokens of reference material. The cartridge provides it dynamically, ranked, and within budget.
...
Context composition lets you combine multiple `.rlat` cartridges at search time without merging them into a single file. Think of it like mixing audio tracks: each cartridge stays independent, but your query hears them all together.

Build one cartridge for your docs, another for your codebase, another for compliance policies, and mix them at query time however the situation demands. No rebuild required.
...
Use the `--with` flag to layer additional cartridges into a single search. Results come back tagged with which cartridge they came from.

```bash
rlat search docs.rlat "auth flow" --with code.rlat --with runbooks.rlat
```

Each result includes a cartridge label, so you always know whether an answer about OAuth middleware came from the documentation, the source code, or an operational runbook.
...
Projection lets you view one knowledge base through the lens of another. "Show me code, but only the parts relevant to compliance" is a natural-language way of saying "project my codebase through my compliance cartridge."

```bash
rlat search code.rlat "data handling" --through compliance.rlat
```

Under the hood, this uses orthogonal projection: the compliance cartridge defines a subspace of meaning, and the code cartridge's results are projected into that subspace. Anything in the code that has no compliance relevance falls to near-zero.

### Can I see what changed between two versions?

Yes. The `--diff` flag creates a queryable semantic diff between two cartridges. This is not a file-level diff -- it is a field you can ask questions about.
...
The `^` operator creates a contradiction field -- a semantic space that is strong where two cartridges diverge and weak where they agree.
...
### Can different cartridges have different LLM injection modes?

Yes. Each cartridge in a composition can control how its knowledge interacts with the LLM.

You might want your general docs to use **augment** mode (the LLM can blend its own knowledge with rlat results) but require your compliance cartridge to use **constrain** mode (the LLM must only use what rlat provides).
...
Build cartridges at regular intervals (weekly, monthly, per-release) and query the evolution. The temporal diff chain shows semantic shifts at each step. The knowledge trend query tells you whether a topic is growing, stable, or shrinking across the sequence.
...
Algebraic access control creates role-specific views of a cartridge using orthogonal projection. This is auditable: the projection produces a certificate that proves exactly what was hidden. The math guarantees that `visible + hidden = original`, so nothing is lost -- it is partitioned, not deleted.
...
Then query it like a single cartridge:
...
|-----------|-------------------|
| 2-cartridge warm query | ~5-25ms over single cartridge |
| 5-cartridge warm query | ~15-60ms over single cartridge |
| Topic sculpting (suppress) | Negligible (single rank-1 update) |
| Cold first query per cartridge | ~200-500ms for load, then warm |

The cold-load cost is a one-time penalty per cartridge per session. Once loaded, subsequent queries add only milliseconds.

### How do I set up automatic cartridge discovery for my assistant?
...
This builds a cartridge, generates a primer, creates a `.rlat/manifest.json` (machine-readable index of all cartridges), updates `.mcp.json` to load the MCP server, and injects a cartridge section into `CLAUDE.md` with query recipes.

Once set up, your assistant can call `rlat_discover` (MCP tool) to list available cartridges with freshness status, and `rlat_freshness` to check if a rebuild is needed. The assistant no longer needs to guess which cartridge to use — it reads the manifest automatically.
